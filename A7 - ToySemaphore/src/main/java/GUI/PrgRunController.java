...
public class PrgRunController implements Initializable {
    ...
    Label lblSemTable;
    @FXML
    TableView<HashMap.Entry<Integer, Triplet<Integer, ArrayList<Integer>, Integer>>> semaphoreTable;
    @FXML
    TableColumn<HashMap.Entry<Integer,Triplet<Integer, ArrayList<Integer>, Integer>>, String>semaphoreTableAddress;
    @FXML
    TableColumn<HashMap.Entry<Integer,Triplet<Integer, ArrayList<Integer>, Integer>>, String> semaphoreTableValue;
    @FXML
    TableColumn<HashMap.Entry<Integer,Triplet<Integer, ArrayList<Integer>, Integer>>, String> semaphoreTableArray;
    @FXML
    ...
    public void setSemaphoreTable() throws Exception{
        if (myController.getRepo().getPrgList().size() > 0) {
            ObservableList<HashMap.Entry<Integer, Triplet<Integer, ArrayList<Integer>, Integer>>> semaphoreTableList = FXCollections.observableArrayList();
            semaphoreTableAddress.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getKey())));
            semaphoreTableValue.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getValue().getValue0())));
            semaphoreTableArray.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getValue1().toString()));
